---
status: done
progress: "Done 2026-05-02. deepscan registered in hub: manifest entry + README catalog row + components.md row + 2-layer validators pass."
---

# 0003 -- Onboard `deepscan` Plugin

Single-plugin execution of the reusable `0002-onboard-additional-plugins.md` workflow. Adds the `deepscan` plugin to the hub catalog. One execution of this plan = one plugin added.

## Goal

Insert the `deepscan` entry into `.claude-plugin/marketplace.json`, sync the README catalog and `docs/architecture/components.md` Section 2, and clear both validator layers. No schema changes; no new requirements; no new ADRs.

## Eligibility Summary

`deepscan` is verdict `READY` per `temp/deepscan-eligibility.md` (derived from `temp/eligibility-survey-CORRECTED.md`, the combined survey produced before 0002 was forked into per-plugin plans):

- Lives at `github.com/idnotbe/deepscan`; visibility PUBLIC; default branch active.
- Upstream `.claude-plugin/plugin.json` exists and is well-formed; `name == "deepscan"`, non-empty `description`, declares `skills`/`keywords`/`homepage`.
...
- Plugin `name` does not collide with existing entries (`claude-code-guardian`, `vibe-check`).
...
- [v] Plugin lives at `github.com/idnotbe/deepscan` (REQ-PLUGIN-ENTRY-002).
...
- [v] Plugin name equals the upstream `plugin.json.name` exactly -- both are `deepscan` (REQ-PLUGIN-ENTRY-004).
...
- [v] Confirm the eligibility-survey result for `deepscan` is `READY` (recorded in `temp/deepscan-eligibility.md`; the combined master survey at `temp/eligibility-survey-CORRECTED.md` is retained as audit trail).
...
- [v] Draft the proposed `marketplace.json` entry, README catalog row, and `components.md` Section 2 row in `temp/0003-onboard-deepscan-phase0-drafts.md`.
- [v] Write gap list and impact assessment to `temp/0003-onboard-deepscan-phase0-alignment.md`.

## Phase 1: Add Manifest Entry

- [v] Insert the `deepscan` entry into `.claude-plugin/marketplace.json` at the correct alphabetical position by `name` (REQ-PLUGIN-ENTRY-005). Position: between `claude-code-guardian` and `humanizer` once `humanizer` lands; absent that, between `claude-code-guardian` and `vibe-check`.
- [v] Use the entry exactly as defined in `temp/0003-onboard-deepscan-phase0-drafts.md`. Confirm bare `https://github.com/idnotbe/deepscan.git` source form. The entry MUST NOT include `ref` or `sha` -- both are forbidden outright by REQ-PLUGIN-ENTRY-002 (ADR-002). Any exception requires a prior amendment to the requirements document and/or a new ADR; it cannot be approved inside this plan.
...
- [v] Insert the `deepscan` row into the README catalog table at the correct alphabetical position matching `.claude-plugin/marketplace.json` (REQ-PLUGIN-ENTRY-005). Name and description MUST be verbatim copies of the manifest entry. Use the row text from `temp/0003-onboard-deepscan-phase0-drafts.md`.

## Phase 3: Update Architecture Docs

- [v] Insert the `deepscan` entry into `docs/architecture/components.md` Section 2 (catalog enumeration) at the correct alphabetical position matching `.claude-plugin/marketplace.json` (REQ-PLUGIN-ENTRY-005). Use the bullet text from `temp/0003-onboard-deepscan-phase0-drafts.md`.
...
- [v] Manual smoke test: from a clean Claude Code session, run `/plugin marketplace add idnotbe/claude-plugins` (or `/plugin marketplace update idnotbe` if already added) and verify `deepscan` appears in `/plugin install` and installs cleanly.

## Phase F-1: Docs Sync Gate

- [v] Verify README catalog list, `docs/architecture/components.md` Section 2, and `marketplace.json` are in lockstep (order + names + descriptions identical for `deepscan`).
- [v] Re-run `claude plugin validate .` AND `bash tests/validate_marketplace.sh`.
- [v] If any in-flight plan is touched (e.g., 0004-onboard-humanizer or 0005-onboard-prd-creator staged the same alphabetical neighborhood), add a cross-plan impact warning per `action-plans/README.md`.
...
- Onboarding `humanizer` or `prd-creator` -- handled by sibling plans `0004-onboard-humanizer.md` and `0005-onboard-prd-creator.md`.
...
- If the eligibility check unexpectedly fails (e.g., the upstream is force-pushed and `plugin.json` disappears), set `status: blocked` with the unblock condition (`upstream idnotbe/deepscan re-publishes .claude-plugin/plugin.json`) and a next review date.
